...
def report_basic_data_properties(csv_file):
    ...
    values = ( number_of_rows, names_cols)
    return values
report_basic_data_properties("townships_with_dist_and_elev.csv")
...
def get_name_of_town_with_highest_elevation(csv_file=None, elev_col="elev_metres"):
    ...
    maximum_value = df[df[elev_col] == max(df[elev_col])]

    value1 = maximum_value['township']
    ...
    # return the answer
    return str(value1.values)
get_name_of_town_with_highest_elevation(csv_file="townships_with_dist_and_elev.csv", elev_col="elev_metres")
...
translate_to_french_for_dunstan(sentence= "big data data analytics with programming is fun")
...
def get_table_rows_from_webpage(url=None):
    ...
    html = response.text
    beautisoup = BeautifulSoup(html, 'lxml')

    # Now get all table rows using the tr tag
    tb_rows = beautisoup.find_all('tr') 

    # return the table rows
    return tb_rows
get_table_rows_from_webpage(url="https://lemobileaukamer.com/nouveaux-tarifs-orange-money-cameroun-juillet-2018")
